...
pipeline {
    ...
    stages {
        ...
        stage('Add git tag') {
            ...
            steps {
                sh 'git clone https://$git_credentials_USR:$git_credentials_PSW@github.com/TBoonX/ontology.git repo_clon && cd repo_clon'
                sh 'git config user.email && git config user.user'
